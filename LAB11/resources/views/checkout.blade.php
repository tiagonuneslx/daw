            ...
            <div class="center-block order-details" style="margin: 0 300px">
                ...
                <div class="order-summary">
                    ...
                    <div class="order-col">
                        <div><strong>TOTAL</strong></div>
                        <div><strong
                                    class="order-total">{{empty($cart) ? 0 : array_reduce($cart, function($carry, $product){return $carry + $product->price * $product->quantity;})}}
                                €</strong></div>
                    </div>
                </div>
                @if($user_id !== -1)
                    @if(!empty($cart))
                        <form action="{{action("Store@checkoutAction")}}" method="post">
                            {{csrf_field()}}
                            <button type="submit"
                                    class="primary-btn order-submit disabled" style="width: 100%">Confirmar
                                encomenda
                            </button>
                        </form>
                    @else
                        <span
                                class="primary-btn order-submit" style="opacity: 0.4">Carrinho Vazio</span>
                    @endif
                @else<a href="{{action("Store@login")}}" class="primary-btn order-submit">Login</a>
                @endif
            </div>
            <!-- /Order Details -->
        </div>
